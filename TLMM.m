...
for dummy_var_2 = 1:experiment
    ...
    w_LMS = weight_update; % setting the weight update vector equal to the initial guess which is all zero vector
    ...
    for dummy_var = 1:iteration
        ...
        w_TLMM = w_TLMM + mu_TLMM*((norm(w_TLMM)^2*e_i_TLMM*u_i'+e_i_TLMM^2*w_TLMM)/norm(w_TLMM)^4); % updating the adaptive filter after finding the error using LMS algorithm
        
        %calculation of the parameter
        mean_square_deviation(dummy_var) = norm(w_LMS-filter_weights)^2; % mean square deviation calculation
        excess_mean_square_error(dummy_var) = norm(e_i_LMS)^2; % Excess Mean Square Error calculation
