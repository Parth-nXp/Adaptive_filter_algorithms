...
mu_LMF = step_size; % step length of the MCC update method
...
for dummy_var_2 = 1:experiment
    ...
    w_LMF = weight_update; % setting the weight update vector equal to the initial guess which is all zero vector
    ...
    for dummy_var = 1:iteration
        ...
        e_i_LMF = (d_i -u_i*w_LMF); % finding error between desired output and filter output to update adaptive filter
        w_LMF = w_LMF + mu_LMF*e_i_LMF*u_i'; % updating the adaptive filter after finding the error using LMF algorithm
        
        %calculation of the parameter
        mean_square_deviation(dummy_var) = norm(w_LMF-filter_weights)^2; % mean square deviation calculation
        excess_mean_square_error(dummy_var) = norm(e_i_LMF)^2; % Excess Mean Square Error calculation
