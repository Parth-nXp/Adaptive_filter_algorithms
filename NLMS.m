...
epsilon_NLMS = 0.001; % value of the epsiolon for NLMS algorithm

wait_bar = waitbar(0,'Starting processing');
mu_NLMS = step_size; % step length of the MCC update method
...
for dummy_var_2 = 1:experiment
    ...
    w_NLMS = weight_update; % setting the weight update vector equal to the initial guess which is all zero vector
    ...
    for dummy_var = 1:iteration
        ...
        mean_square_deviation(dummy_var) = norm(w_NLMS-filter_weights)^2; % mean square deviation calculation
        excess_mean_square_error(dummy_var) = norm(e_i_NLMS)^2; % Excess Mean Square Error calculation
