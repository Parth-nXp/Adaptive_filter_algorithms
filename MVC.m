...
for dummy_var_2 = 1:experiment
    ...
    w_MVC = weight_update; % setting the weight update vector equal to the initial guess which is all zero vector
    ...
    for dummy_var = 1:iteration
        ...
        e_i_MVC = (d_i -u_i*w_MVC); % finding error between desired output and filter output to update adaptive filter
        w_MVC = w_MVC + mu_MVC*(1/(1+tau*(e_i_MVC)^2)^2)*e_i_MVC*u_i'; % updating the adaptive filter after finding the error using MVC algorithm
        
        %calculation of the parameter
        mean_square_deviation(dummy_var) = norm(w_MVC-filter_weights)^2; % mean square deviation calculation
        excess_mean_square_error(dummy_var) = norm(e_i_MVC)^2; % Excess Mean Square Error calculation
