...
for dummy_var_2 = 1:experiment
    ...
    w_LHCAF = weight_update; % setting the weight update vector equal to the initial guess which is all zero vector
    ...
    for dummy_var = 1:iteration
        ...
        e_i_LHCAF = (d_i -u_i*w_LHCAF); % finding error between desired output and filter output to update adaptive filter
        w_LHCAF = w_LHCAF + mu_LHCAF*tanh(lambda*e_i_LHCAF)*u_i'; % updating the adaptive filter after finding the error using LHCAF algorithm

        %calculation of the parameter
        mean_square_deviation(dummy_var) = norm(w_LHCAF-filter_weights)^2; % mean square deviation calculation
        excess_mean_square_error(dummy_var) = norm(e_i_LHCAF)^2; % Excess Mean Square Error calculation
